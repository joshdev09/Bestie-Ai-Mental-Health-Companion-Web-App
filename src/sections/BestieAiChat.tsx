...
function BestieAiChat() {
    ...
    return (
        ...
        <div className="font-sans h-full w-full flex items-center justify-center p-4">
            <div className="bg-white w-full md:w-full max-w-2xl h-[83vh] md:h-[80vh] rounded-lg shadow-2xl flex flex-col">
                
                {/* Header */}
                <header className="bg-indigo-600 text-white p-4 rounded-t-lg flex items-center shadow-md shrink-0">
                    <h1 className="text-xl font-semibold">Bestie AI</h1>
                </header>

                {/* Chat Log */}
                {/* We apply the ref to this div */}
                <div ref={chatLogRef} className="flex-1 p-4 overflow-y-auto space-y-4">
                    {/* We map over chatHistory, skipping the 'system' message */}
                    {chatHistory.filter(msg => msg.role !== 'system').map((message, index) => (
                        <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : ''}`}>
                            <div className={`p-3 rounded-lg shadow max-w-xs sm:max-w-md ${
                                message.role === 'user'
                                    ? 'bg-indigo-600 text-white'
                                    : 'bg-slate-200 text-slate-800'
                            }`}>
                                {/* Use dangerouslySetInnerHTML to render <br> tags for newlines */}
                                <p dangerouslySetInnerHTML={{ __html: formatMessage(message.parts[0].text) }} />
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {isLoading && (
                    <div className="p-4 shrink-0">
                        <div className="flex items-center">
                            <div className="bg-slate-200 p-3 rounded-lg flex items-center shadow">
                                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-slate-700"></div>
                                <p className="ml-2 text-slate-700 text-sm">Thinking...</p>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {/* Input Area */}
                <form onSubmit={handleSubmit} className="p-4 rounded-b-lg flex items-center shrink-0">
                    <input 
                        type="text" 
                        value={userInput}
                        ...
                        autoComplete="off"
                        disabled={isLoading}
                        className="flex-1 p-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition disabled:opacity-50"
                    />
                    <button 
                        type="submit" 
                        disabled={isLoading}
                        className="ml-3 bg-indigo-600 text-white p-3 rounded-lg shadow hover:bg-indigo-700 transition active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
